...
export default async function Home() {
  ...
  return (
    <main className="flex flex-col min-h-screen">
      {/* Hero */}
      <section className="flex-1 flex flex-col items-center justify-center text-center px-4 py-24 bg-gradient-to-b from-indigo-950/40 to-[#0f0f14]">
        ...
        <h1 className="text-5xl font-bold text-white mb-4 max-w-2xl leading-tight">
          Agenda tu cita en{' '}
          <span className="text-indigo-400">{config?.negocio_nombre ?? 'Agendalo'}</span>
        </h1>
        ...
        <Link
          href="/reservar"
          className="bg-indigo-600 hover:bg-indigo-500 text-white text-lg font-bold px-8 py-4 rounded-2xl transition shadow-lg shadow-indigo-900/50"
        >
          Reservar ahora
        </Link>
      </section>
      ...
      {/* Cómo funciona */}
      <section className="py-20 px-4 bg-[#13131a] border-t border-white/5">
      ...
      {/* Contacto */}
      {config && (
        <section className="py-10 px-4 bg-[#0f0f14] border-t border-white/5">
          <div className="max-w-2xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-6 text-sm text-slate-500">
            ...
            {config.negocio_direccion && <span>{config.negocio_direccion}</span>}
          </div>
        </section>
      )}
